      ...
      <div class="block-center mt-xl wd-xl">
         ...
         <div class="panel panel-dark panel-flat">
            ...
            <div class="panel-body">
               <p class="text-center pv">FORGOT YOUR PASSWORD</p>
               <?php
                  if (!empty($status) && $status == 'not found') {
                  echo '<p class="error tC">Username is not found..!</p> <br/>';
                  }elseif(!empty($status) && $status == 'send'){
                  echo '<p class="error tC">Email is sent to your Email-ID..!</p> <br/>';
                  }elseif(!empty($status) && $status == "fail"){
                  echo '<p class="error tC">Somrthing went wrong, try after sometime..!</p> <br/>';
                  }
                  ?> 
               <form role="form" action="<?= site_url('Admin/forgot_pswd') ?>"  class="mb-lg" name="login_form" method="post">
                  <div class="form-group has-feedback">
                     <input name="email" id="email"  placeholder="Enter email" autocomplete="off" required class="form-control">
                     <span class="fa fa-envelope form-control-feedback text-muted"></span>
                  </div>
                  <div class="clearfix">
                     <div class="pull-right"><a href="<?= site_url('Admin/index') ?>" class="text-muted">Login Here</a>
                     </div>
                  </div>
                  <input type="submit" class="btn btn-primary mt-lg" name="forgotpswd" value="Submit"/>
               </form>
